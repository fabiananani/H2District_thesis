...
%% INPUT PARAMETERS
 ...
 % general inputs
    ...
    MaxSimTime      = 800;     % Maximum time for MILP solver                 [s]      
 ...
 % Costs and Efficencies 
    ...
    c_h2        = 8;                                     % cost of hydrogen [euro/kg]
...
% McCormick method parameters 
m_min = (max(P_load)/eff_FC/HHV*3600); % [kg/h]
m_max = (S_FC_max/eff_FC/HHV*3600); % [kg/h]
eff_min = 0; % eff_FC/2; 
eff_max = eff_FC;
...
% OPERATIONAL decision variables
    ...
    w_hours        = optimvar('w_hours',nHours,'LowerBound',0,'UpperBound',8760*2); %cumulative summation of FC_On
    
    % efficency with degradation 
    eff_deg        = optimvar('eff_deg', nHours, 'LowerBound', eff_min, 'UpperBound', eff_max); 
    m_flow_H2      = optimvar('m_flow_H2',nHours,'LowerBound',m_min,'UpperBound',m_max); % [kg/h]
    % z              = optimvar ('z',nHours,'LowerBound',0);
    ...
    sizingprob.Constraints.SumCum_0                  = w_hours(1) == 0;
    sizingprob.Constraints.SumCum_update             = w_hours(idxHr2ToEnd) == w_hours(idxHr2ToEnd-1) + FC_On(idxHr2ToEnd);
   
    % constraints on efficency with degradation 
    sizingprob.Constraints.FC_EfficiencyDegradation      = eff_deg == eff_FC - Const * w_hours;
    sizingprob.Constraints.FC_massflowrate               = m_flow_H2 == P_FC./HHV./eff_deg*3600;
    % % McCormick coonstraints 
    % sizingprob.Constraints.McCormick1                  = z >= m_min*eff_deg + m_flow_H2 * eff_min - m_min*eff_min;
    % sizingprob.Constraints.McCormick2                  = z <= m_max*eff_deg + m_flow_H2 * eff_min - m_max*eff_min;
    % sizingprob.Constraints.McCormick3                  = z >= m_max*eff_deg + m_flow_H2 * eff_max - m_max*eff_max;
    % sizingprob.Constraints.McCormick4                  = z <= m_min*eff_deg + m_flow_H2 * eff_max - m_min*eff_max;
    % sizingprob.Constraints.McCormick5                  = z*HHV/3600== P_FC; 
...
S_FC_opt        = evaluate(S_FC,solution);                                       % [kW]
...
m_flow_H2_opt   = evaluate(m_flow_H2,solution);
m_flow_H2_nom   = P_FC_opt./eff_FC./HHV.*3600; 
eff_deg_opt     = solution.eff_deg; 
tot_hours     = sum(FC_On_opt);    
w_hours_opt   = solution.w_hours;
...
figure('Position', [100, 100, 1800, 500]); % [x, y, width, height]
plot(tt,m_flow_H2_opt(startS:finishS),'LineWidth',0.5);
hold on
plot(tt,m_flow_H2_nom(startS:finishS),'LineWidth',0.5);
hold on
ylabel('Mass flow rate [kg/h]',FontWeight='bold')
% xlabel('Time [h]',FontWeight='bold')
legend ('Mass flow with degradation','Mass flow nominal',Location='eastoutside')
ax = gca;
ax.FontSize = font;
ylim([0 2*max(P_FC_opt/eff_FC./HHV.*deltat)])
xlim([tt(1) tt(end)])
grid on

%% SELECTED WEEK PLOT,summer and winter
% functions directory
